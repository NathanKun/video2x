...
public final class ImageKernel extends Kernel {
    ...
    public void updateBufferedImage() {
        if (!isForwardPass) {
			Range range = null;
			if (colorData.length % 256 == 0) {
				range = Range.create(colorData.length, 256);
			} else if (colorData.length % 128 == 0) {
				range = Range.create(colorData.length, 128);
			} else if (colorData.length % 64 == 0) {
				range = Range.create(colorData.length, 64);
			} else if (colorData.length % 32 == 0) {
				range = Range.create(colorData.length, 32);
			} else {
				range = Range.create(colorData.length, 16);
			}
            runtype[0] = -1;
            this.execute(range);
